...
const DataTable = ({ filteredCollege, sortedData }) => {
  ...
  useEffect(() => {
    ...
    if (filteredCollege?.length > 0) setDataToRender(filteredCollege);
  }, [filteredCollege, sortedData]);

  const fetchMoreData = () => {
    setTimeout(() => {
      const newData = [];
      const startIndex = dataToRender.length % collegesData.length;
      for (let i = startIndex; i < startIndex + 10; i++) {
        const college = collegesData[i % collegesData.length];
        newData.push(college);
      }
      setDataToRender((prevData) => [...prevData, ...newData]);
      setHasMore(newData.length > 0); // Update hasMore based on fetched data
            ...
            {dataToRender?.map((college, index) => (
              <tr key={index} className={college.featured ? "featured" : ""}>
                ...
                <td
                  ...
                  style={{
                    paddingTop: college.featured ? "23px" : "10px",
                  }}
                >
                  {college.featured && (
